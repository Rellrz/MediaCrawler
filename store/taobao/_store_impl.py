...
class TaobaoFileStoreImplement(AbstractStore):
    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.writer = AsyncFileWriter(
            platform="tb", crawler_type=crawler_type_var.get()
        )

    async def store_content(self, content_item: Dict) -> None:
        raise NotImplementedError("淘宝平台当前仅保存商品评论")
    ...
    async def store_creator(self, creator: Dict) -> None:
        raise NotImplementedError("淘宝平台当前不保存创作者信息")


class TaobaoExcelStoreImplement:
